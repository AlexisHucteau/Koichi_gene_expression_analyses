...
Prepare_Cytoscape_network <- function(Big_Network = igraph_PPI_TF_target_Network, DEG_analysis, TF_analysis, logFC_treshold = 0.75){
  ...
  filtered_reversed_graph <- reverse_edge(filtered_graph, E(filtered_graph))
  
  eigen_centrality_result <- eigen_centrality(filtered_graph, directed = F)$vector
  eigen_centrality_directed_result <- eigen_centrality(filtered_graph, directed = T)$vector
  eigen_centrality_reverse_result <- eigen_centrality(filtered_reversed_graph, directed = T)$vector
  
  page_rank_result <- igraph::page.rank(filtered_graph, directed = F)$vector
  page_rank_directed_result <- igraph::page.rank(filtered_graph, directed = T)$vector
  page_rank_reverse_result <- igraph::page.rank(filtered_reversed_graph, directed = T)$vector
  ...
  features <- merge(features, eigen_centrality_directed_result, by.x = "Gene", by.y = 0, all = T)
  colnames(features)[ncol(features)] <- "Eigen_directed_centrality"
  features <- merge(features, eigen_centrality_reverse_result, by.x = "Gene", by.y = 0, all = T)
  colnames(features)[ncol(features)] <- "Eigen_reverse_centrality"
  features <- merge(features, page_rank_result, by.x = "Gene", by.y = 0, all = T)
  colnames(features)[ncol(features)] <- "Page_rank"
  features <- merge(features, page_rank_directed_result, by.x = "Gene", by.y = 0, all = T)
  colnames(features)[ncol(features)] <- "page_rank_directed"
  features <- merge(features, page_rank_reverse_result, by.x = "Gene", by.y = 0, all = T)
  colnames(features)[ncol(features)] <- "Page_rank_reverse"
  ...
  set(features,which(is.na(features[["Eigen_directed_centrality"]])),"Eigen_directed_centrality",0)
  set(features,which(is.na(features[["page_rank_directed"]])),"page_rank_directed",0)
  set(features,which(is.na(features[["Eigen_reverse_centrality"]])),"Eigen_reverse_centrality",0)
  set(features,which(is.na(features[["Page_rank_reverse"]])),"Page_rank_reverse",0)
  
  features$TF <- ifelse(features$nes == 0, F, T)
  
  res <- list("features" = features,
              "reversed_network" = filtered_reversed_graph,
...
Features_NR_R$reversed_network %>%
  igraph::as_data_frame() %>% 
  write.table("Results/Tables/filtered_PPI_TF_NR_R_reversed_network.tsv", sep = "\t", row.names = F, quote = F)
...
Features_REL_R$reversed_network %>% 
  igraph::as_data_frame() %>% 
  write.table("Results/Tables/filtered_PPI_TF_REL_R_reversed_network.tsv", sep = "\t", row.names = F, quote = F)
